    ...
    <div className="space-y-4">
      ...
      <div className="mt-6 p-4 bg-white rounded-lg border border-slate-200">
        ...
        {(() => {
          ...
          // Build chart data with ALL components visible
          const costDriverData = [];

          // Add scanning if enabled (check if value > 0)
          if (model.P_scanning > 0) {
            costDriverData.push({
              name: 'Document Scanning',
              value: parseFloat(scanningPercent),
              label: `${scanningPercent}%`
            });
          }

          // Always add OCR (unless scanning service is used, which includes OCR)
          if (!inputs.includeScanningService && model.P_OCR > 0) {
            costDriverData.push({
              name: 'OCR Processing',
              value: parseFloat(ocrPercent),
              label: `${ocrPercent}%`
            });
          }

          // Always add AI Extraction
          if (model.P_LLM > 0) {
            costDriverData.push({
              name: 'AI Extraction',
              value: parseFloat(llmPercent),
              label: `${llmPercent}%`
            });
          }

          // Always add Manual Review
          if (model.P_manual_eng > 0) {
            costDriverData.push({
              name: 'Manual Review',
              value: parseFloat(manualPercent),
              label: `${manualPercent}%`
            });
          }
          ...
          costDriverData.push({
            ...
            value: parseFloat(opexPercent),
            label: `${opexPercent}%`
          });

          const COLORS = ['#10b981', '#06b6d4', '#8b5cf6', '#ec4899', '#3b82f6', '#f59e0b'];
          ...
          return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              ...
              {/* Cost Driver Explanations - Dynamic based on chart data */}
              <div className="space-y-4 text-sm">
                {costDriverData.map((driver, idx) => {
                  // Define descriptions for each component
                  const descriptions = {
                    'Document Scanning': 'Physical document preparation, scanning, and AI-ready output',
                    'OCR Processing': 'Convert scanned images to searchable text',
                    'AI Extraction': 'Multi-model AI processing to extract structured data',
                    'Manual Review': 'Quality assurance and conflict resolution — scales with data quality',
                    'Platform Development': 'One-time investment in custom platform build, security hardening, and testing',
                    'Annual Operations': 'Predictable ongoing costs — infrastructure, support, and maintenance'
                  };

                  const isLast = idx === costDriverData.length - 1;

                  return (
                    <div key={driver.name} className={isLast ? '' : 'pb-3 border-b border-slate-200'}>
                      <div className="flex items-center gap-2 mb-1">
                        <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: COLORS[idx] }}></div>
                        <span className="font-semibold text-slate-900">{driver.name} (~{driver.label})</span>
                      </div>
                      <p className="text-slate-600 text-xs ml-5">{descriptions[driver.name]}</p>
                    </div>
                  );
                })}
              </div>
