 ...
 */
const ProfessionalReport = ({ model, inputs, scenario, reportVariant, SCENARIO_CONFIGS }) => {
  ...
  const InternalReport = () => {
    // Calculate cost driver percentages - DISAGGREGATED by actual components
    const totalPrice = model.capexOneTimePrice + model.opexAnnualPrice;

    // Ingestion components (separate, not lumped)
    const scanningPercent = ((model.P_scanning / totalPrice) * 100).toFixed(0);
    const ocrPercent = ((model.P_OCR / totalPrice) * 100).toFixed(0);
    const llmPercent = ((model.P_LLM / totalPrice) * 100).toFixed(0);
    const manualPercent = ((model.P_manual_eng / totalPrice) * 100).toFixed(0);
    ...
    // Build cost driver data array (same logic as ClientQuoteSummary)
    const costDriverData = [];

    if (model.P_scanning > 0) {
      costDriverData.push({
        name: 'Document Scanning',
        percent: scanningPercent,
        description: 'Physical document preparation, scanning, and AI-ready output',
        color: '#10b981'
      });
    }

    if (!inputs.includeScanningService && model.P_OCR > 0) {
      costDriverData.push({
        name: 'OCR Processing',
        percent: ocrPercent,
        description: 'Convert scanned images to searchable text',
        color: '#06b6d4'
      });
    }
    ...
    if (model.P_LLM > 0) {
      costDriverData.push({
        name: 'AI Extraction',
        percent: llmPercent,
        description: 'Extract structured data from documents using large language models',
        color: '#8b5cf6'
      });
    }

    if (model.P_manual_eng > 0) {
      costDriverData.push({
        name: 'Manual Review',
        percent: manualPercent,
        description: 'Quality assurance and exception handling by domain experts',
        color: '#ef4444'
      });
    }
    ...
    costDriverData.push({
      ...
      description: 'One-time investment in custom platform build, security hardening, and testing',
      color: '#3b82f6'
    });
    ...
    costDriverData.push({
      ...
      percent: opexPercent,
      description: 'Predictable ongoing costs — infrastructure, support, and maintenance',
      color: '#f59e0b'
    });
      ...
      <div className="page" style={{ padding: '40px' }}>
        ...
        <div style={{ marginTop: '32px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#475569', lineHeight: '1.7', marginBottom: '20px' }}>
            {costDriverData.map((driver, index) => (
              <div
                key={driver.name}
                style={{
                  marginBottom: index < costDriverData.length - 1 ? '16px' : '0',
                  paddingBottom: index < costDriverData.length - 1 ? '12px' : '0',
                  borderBottom: index < costDriverData.length - 1 ? '1px solid #e2e8f0' : 'none'
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                  <div style={{ width: '12px', height: '12px', backgroundColor: driver.color, borderRadius: '2px', marginRight: '8px' }}></div>
                  <strong style={{ color: '#1e293b' }}>{driver.name} (~{driver.percent}%)</strong>
                </div>
                <p style={{ margin: '0 0 0 20px', fontSize: '12px' }}>{driver.description}</p>
              </div>
            ))}
          </div>
      ...
      <div className="page" style={{ padding: '40px' }}>
        ...
        <div style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px' }}>
          ...
          <ul style={{ fontSize: '11px', color: '#1e3a8a', lineHeight: '1.6', margin: 0, paddingLeft: '20px' }}>
            ...
            <li>Support hours are ring-fenced per team with rollover tracking; additional hours billed at support rate</li>
          </ul>
        </div>
      </div>
  ...
  const ROMReport = () => {
    ...
    if (model.P_LLM > 0) {
      costDriverData.push({
        name: 'AI Extraction',
        percent: llmPercent,
        description: 'Multi-model AI processing to extract structured data',
        color: '#8b5cf6'
      });
    }
    ...
    if (model.P_manual_eng > 0) {
      costDriverData.push({
        name: 'Manual Review',
        percent: manualPercent,
        description: 'Quality assurance and conflict resolution — scales with data quality',
        color: '#ec4899'
      });
    }
    ...
    return (
      <>
        ...
        <div className="page" style={{ padding: '40px' }}>
          ...
          <div style={{ marginBottom: '40px' }}>
            <h4 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '8px', color: '#1e293b' }}>
              ROM Estimated Costs
            </h4>
